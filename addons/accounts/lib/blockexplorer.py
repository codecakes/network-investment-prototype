...
def validate(amt, src_addr, addr, txn_id, coin="btc"):
    ...
    if coin == "btc":
        ...
        if txn_res and addr_res:
            result = True if (txn_res.get("status", None) == "1" and addr_res.get("status", None) == "1") else False
    elif coin == "xrp":
        j_txn = txn_res.json()
        j_addr = addr_res.json()
        if j_addr.result != "error" and j_txn.result != "error":
            payments = j_txn.payments
            result = any(divide_conquer(payments, 0, len(payments) - 1, lambda payment: is_valid_xrp_paid(amt, txn_id, addr, src_addr, payment)))
    result = False
    return {addr: result}
